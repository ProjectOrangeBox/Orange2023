...
const app = {
    ...
    methods: {
        ...
        autoLoad() {
            // for each data-autoload true element
            $('#' + app.rootElement.id + ' [data-autoload="true"]').each(function () {
                // grab the other data attributes
                // grab the url [*required], property [records], method [get]
                let property = this.dataset.property || 'records';

                let method = this.dataset.method || 'get';

                app.methods.model(this.dataset.url, property, method);
            });
        },
