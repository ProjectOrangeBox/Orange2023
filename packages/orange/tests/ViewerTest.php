...
final class ViewerTest extends unitTestHelper
{
    ...
    public function testAddPath(): void
    {
        ...
        $this->assertTrue(in_array(WORKINGFOLDER . '/views', $this->instance->search->listDirectories()));
    }

    public function testAddPaths(): void
    {
        $this->instance->search->addDirectories([
            WORKINGFOLDER . '/directorySearch/foo',
            WORKINGFOLDER . '/directorySearch/bar'
        ]);

        $this->assertTrue(in_array(WORKINGFOLDER . '/directorySearch/foo', $this->instance->search->listDirectories()));
        $this->assertTrue(in_array(WORKINGFOLDER . '/directorySearch/bar', $this->instance->search->listDirectories()));
    }
    ...
    {
        $this->instance->search->flushDirectories(true)->addDirectory(WORKINGFOLDER . '/views/errors');
        $this->assertEquals(WORKINGFOLDER . '/views/errors/cli/404.php', $this->instance->search->findFirst('cli/404'));
    }
}
