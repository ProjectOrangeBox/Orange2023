...
use orange\framework\exceptions\Security as ExceptionsSecurity;
...
class Security extends Singleton implements SecurityInterface
{
    ...
    public static function createKeys(string $publicKeyFile, string $privateKeyFile, int $bits = 2048, int $type = OPENSSL_KEYTYPE_RSA): bool
    {
        ...
        if (!is_writable(dirname($publicKeyFile))) {
            throw new FolderNotWritable('Folder [' . dirname($publicKeyFile) . '] is not writable.');
        }

        if (!is_writable(dirname($privateKeyFile))) {
            throw new FolderNotWritable('Folder [' . dirname($privateKeyFile) . '] is not writable.');
        }
        ...
        if (openssl_pkey_export_to_file($privateKey, $privateKeyFile)) {
            ...
            $success = file_put_contents($publicKeyFile, $publicKey['key']) > 0;
        }
