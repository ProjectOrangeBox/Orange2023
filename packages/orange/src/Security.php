...
class Security extends Singleton implements SecurityInterface
{
    ...
    protected int $hmacMinimumLength = 0;
    ...
    {
        $this->config = $this->mergeWithDefault($config, 'security', false);
    ...
    public function setHmacMinimumLength(int $min): void
    {
        $this->hmacMinimumLength = $min;
    }

    public function setHmacHashingAlgorithm(string $algo): void
    {
        $this->hmacHashingAlgorithm = $algo;
    }
    ...
    {
        return sha1_file($this->getKeyFile('public'));
    }
