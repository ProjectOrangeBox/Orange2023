...
use orange\framework\interfaces\CookieInterface;
...
class Cookie extends Singleton implements CookieInterface
{
    ...
    {
        $this->config = $this->mergeWithDefault($config, 'cookie');
    ...
    {
        $this->output->header('Set-Cookie: ' . $this->generateHeader($name, $value, $expires, $path, $domain, $secure, $httpOnly, $sameSite), false);
    }

    public function remove(string $name, string $path = '', string $domain = ''): void
    {
        // remove the matching regular expression
        $this->output->removeHeaders('/^Set-Cookie: ' . $name . '=(.*)$/m');
