...
class ViewMerge extends ViewerAbstract implements ViewerInterface
{
    ...
    {
        return $this->parse(file_get_contents($__viewFilePath), $__dataArray, [$this, 'pluginCallBackHandler']);
    }

    public function pluginCallBackHandler($name, $parameters, $content)
    {
        // !TODO
    }
     ...
     */
    protected function parse(string $text, array $data = [], $callback = false): string
    {
        $this->setupRegex();
    ...
    protected function processConditionVar(array $match): string
    {
        ...
        if (
            in_array(strtolower($var), ['true', 'false', 'null', 'or', 'and']) or
    ...
    protected function parsePhp(string $text): string
    {
        ...
        if ($result === false) {
            ...
            throw new Parser($output . str_replace(['?>', '<?php '], '', $text));
        }
