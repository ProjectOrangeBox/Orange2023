...
class Input extends Singleton implements InputInterface
{
    ...
    {
        $this->config = $this->mergeWithDefault($config, 'input', false);
    ...
    {
        if (!$this->isHttps) {
            // hacky - dynamically load output
            container()->output->redirect('https://' . $this->extract('server', 'http_host') . $this->extract('server', 'request_uri'));
        }
    }
