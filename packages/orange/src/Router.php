...
class Router extends Singleton implements RouterInterface
{
    ...
    protected function __construct(array $config, InputInterface $input)
    {
        ...
        $this->config = Application::mergeDefaultConfig($config, __DIR__ . '/config/routes.php', false);
    ...
    {
        log::msg('DEBUG', __METHOD__ . ' uri:' . $requestUri . ' method:' . $requestMethod);
    ...
    public function getUrl(string $searchName, array $arguments = []): string
    {
        ...
        foreach ($this->routes as $record) {
            ...
            if (isset($record['name'], $record['url']) && $this->normalizeName($record['name']) == $searchName && preg_match_all('/\((.*?)\)/m', $record['url'], $matches, PREG_SET_ORDER, 0) !== false) {
