...
class Application
{
    ...
    {
        // recursively replace the defaults with the passed in configuration
        $config = self::mergeDefaultConfig($config, __DIR__ . '/config/application.php', true);
        ...
        $eecv = $config['environment errors config'][ENVIRONMENT] ?? $config['environment errors config']['default'];

        // ok now set those values
        ini_set('display_errors', $eecv['display errors']);
        ini_set('display_startup_errors', $eecv['display startup errors']);
        error_reporting($eecv['error reporting']);
        ...
        // set constants
        if (is_array($container->config->constants)) {
            foreach ($container->config->constants as $name => $value) {
                define($name, $value);
            }
        }
