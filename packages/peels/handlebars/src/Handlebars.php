...
class Handlebars
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->cacheFolder = $this->config['cache folder'];
        ...
        if (!is_dir($this->cacheFolder)) {
            throw new DirectoryNotFound($this->cacheFolder);
        }
 ...
 /* partial & template handling */
    ...
    public function parseView(string $template, bool $isFile): string
    {
        ...
        $compiledFile = $this->cacheFolder . '/' . $this->hbCachePrefix . sha1($template) . '.php';
    ...
    public function run(string $compiledFile, array $data): string
    {
        ...
        try {
            ...
            var_dump($e);
            logMsg('DEBUG', \var_export($e, true));
            echo '</pre>';
            exit(1);
        }
