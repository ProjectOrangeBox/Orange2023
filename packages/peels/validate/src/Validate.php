...
class Validate implements ValidateInterface
{
    ...
    {
        $this->config = Application::mergeDefaultConfig($config, __DIR__ . '/config/validate.php');
    ...
    public function throwErrorOnFailure(bool $bool = true): self
    {
        $this->throwErrorOnFailure = $bool;
    ...
    protected function throwException(): self
    {
        if ($this->throwErrorOnFailure && $this->hasErrors()) {
            // failed http 406 Not Acceptable (ie. a validation error)
            throw new ValidationFailed(implode(PHP_EOL, $this->errorsText()), 406, null, $this->errors);
        }
