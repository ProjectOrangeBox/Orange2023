...
class Remap implements RemapInterface
{
    ...
    public function input(string $method, string $mapping): array
    {
        ...
        $array = $this->inputService->$method();
        ...
        if (!empty($mapping)) {
            ...
            $this->inputService->replace([$method => $array]);
        }
    ...
    {
        logMsg('debug', __METHOD__ . ' input ' . $input);
        ...
        logMsg('debug', __METHOD__ . ' output ' . $input);
