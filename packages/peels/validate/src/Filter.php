    ...
    public function value(mixed $value, string|array $rules): mixed
    {
        if (is_array($rules)) {
            foreach ($rules as $rule) {
                $value = $this->runRule($value, $rule);
            }
        } else {
            $value =$this->runRule($value, $rules);
        }

        return $value;
    }

    protected function runRule(mixed $value, string $rules): mixed
    {
        // throws exception on fail
        // returns value on success
        return $this->validateService->throwErrorOnFailure(true)->input($value, $rules)->value();
    }
}
