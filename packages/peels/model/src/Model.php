...
abstract class Model
{
    ...
    */
    ...
    public function __construct(array $config, PDO $pdo, ValidateInterface $validateService)
    {
        ...
        // throw exceptions for failed rules
        $this->validateService->throwErrorOnFailure(true);
    ...
    public static function getInstance(array $config, PDO $pdo, ValidateInterface $validateService): self
    {
        if (!isset(self::$instance)) {
            ...
            self::$instance = new $extendingClass($config, $pdo, $validateService);
        }
    ...
    public function filterFields(array $fields, string $set): array
    {
    ...
    public function validateFields(string $set, array &$fields): bool
    {
        ...
        // we only need these columns (any additional are removed!)
        $fields = $this->filterFields($fields, $set);

        // validate our record
        $this->validateService->input($fields, $rules)->values();

        // get out our errors
        $fields = $this->validateService->values();
