...
use orange\framework\exceptions\MissingRequiredException;
...
class User implements UserInterface
{
    ...
    {
        $this->config = mergeDefaultConfig($config, __DIR__ . '/config/acl.php');
        $this->acl = $acl;
        $this->sessionService = $sessionService;

        // required value
        if (!isset($this->config['guest user']) || !is_integer($this->config['guest user'])) {
            throw new MissingRequiredException('Invalid Guest User Id');
        }
