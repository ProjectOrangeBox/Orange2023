...
class UserModel extends Model implements UserModelInterface
{
    ...
    {
        $config = mergeDefaultConfig($config, __DIR__ . '/../config/acl.php');

        $this->entityClass = $config['UserEntityClass'] ?? \peels\acl\entities\UserEntity::class;
        ...
        $this->tableJoin = $config['user role table'];

        // I manage the meta model 100%
        $this->userMetaModel = new UserMetaModel(['tablename' => $config['user meta table']], $pdo, $validateService);

        $validateService->throwErrorOnFailure(true);
    ...
    public function update(array $columns): bool
    {
        ...
        if ($errors->hasErrors()) {
            echo $errors->getErrorsAsHtml('<p>', '</p>', PHP_EOL);
