...
use peels\acl\exception\aclException;
...
class RoleModel extends Model implements RoleModelInterface
{
    ...
    public function __construct(array $config, PDO $pdo, ?ValidateInterface $validateService)
    {
        ...
        $this->entityClass = $config['RoleEntityClass'] ?? \peels\acl\entities\RoleEntity::class;

        $config['tablename'] = $this->tablename = $config['role table'];
