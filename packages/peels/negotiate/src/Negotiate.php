...
class Negotiate implements NegotiateInterface
{
    ...
    {
        return $this->getBestMatch($supported, $this->input->server('accept', ''), true, $strictMatch);
    }
    ...
    {
        $match = $this->getBestMatch($supported, $this->input->server('accept-charset', ''), false, true);
    ...
    public function encoding(array $supported = []): string
    {
        ...
        return $this->getBestMatch($supported, $this->input->server('accept-encoding', ''));
    }
    ...
    {
        return $this->getBestMatch($supported, $this->input->server('accept-language', ''), false, false, true);
    }
     ...
     */
    protected function getBestMatch(array $supported, string $header, bool $enforceTypes = false, bool $strictMatch = false, bool $matchLocales = false): string
    {
        if ($supported === []) {
            throw new InvalidValue('supported cannot be empty');
        }
     ...
     */
    protected function matchTypes(array $acceptable, array $supported): bool
    {
     ...
     */
    protected function matchLocales(array $acceptable, array $supported): bool
    {
